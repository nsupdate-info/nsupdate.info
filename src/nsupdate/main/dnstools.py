...
def update_related_hosts(host, fqdn, kind, ipaddr, logger):
        ...
        if rh.available:
            ...
            try:
                ...
                try:
                    ...
                    msg = '%s - related hosts no-change update, ip: %s' % (rh_fqdn, rh_ipaddr)
                    logger.warning(msg)
                    host.register_client_result(msg, fault=True)
                except (DnsUpdateError, NameServerNotAvailable) as e:
                    msg = str(e)
                    if not _delete:
                        msg = '%s - related hosts update that resulted in a dns error [%s], ip: %s' % (
                            rh_fqdn, msg, rh_ipaddr)
                    else:
                        msg = '%s - related hosts deletion that resulted in a dns error [%s]' % (
                            rh_fqdn, msg)
                    logger.error(msg)
